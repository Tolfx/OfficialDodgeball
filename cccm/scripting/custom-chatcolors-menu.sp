...
new String:g_strTag[MAXPLAYERS + 1][255];
...
public SQL_LoadColors(iClient)
{
	...
	if(g_hSQL != INVALID_HANDLE)
	{
		...
		Format(strQuery, sizeof(strQuery), "SELECT hidetag, tagcolor, namecolor, chatcolor, tag FROM cccm_users WHERE auth = '%s'", g_strAuth[iClient]);
...
public OnClientConnected(iClient)
{
	...
	strcopy(g_strTag[iClient], sizeof(g_strTag[]), "");
...
public SQLQuery_LoadColors(Handle:hOwner, Handle:hQuery, const String:strError[], any:iData)
{
	...
	if(SQL_FetchRow(hQuery) && SQL_GetRowCount(hQuery) != 0)
	{
		...
		decl String:tag[255];
		SQL_FetchString(hQuery, 4, tag, sizeof(tag));
		if (!StrEqual(tag, ""))
		{
			strcopy(g_strTag[iClient], sizeof(g_strTag[]), tag);
			CCC_SetTag(iClient, g_strTag[iClient]);
		}
